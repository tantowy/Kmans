...
class Stemming_nazief_adriani {

    protected $CI;

    public function __construct() {
        $this->CI = & get_instance();
        $this->CI->load->model('tabel');
    }

    public function Nazief_Adriani($kata) {
        $kataAsal = $kata;
        $this->CI->load->model('tabel');
        // 1. Cek Kata di Kamus jika Ada SELESAI 
        if ($this->cek_katadasar($kata)) { // Cek Kamus
        ...
        $kata = $this->Del_Derivation_Suffixes($kata);
        //$kata = $this->Del_Inflection_Suffixes($kata);
        if ($this->cek_katadasar($kata) == true) {
            return $kata;
        } else {
    ...
    public function cek_katadasar($kata) {
        $this->CI->load->model('tabel');
        if (($this->CI->tabel->cek_kt_dasar($kata)) == 1) {
            return true;
        } else {
            return false;
        }
    ...
    public function Del_Inflection_Suffixes($kata) {
        ...
        /* if(eregi('([km]u|nya|[kl]ah|pun)$',$kata)){ // Cek Inflection Suffixes
          ...
          if(eregi('([kl t]ah|pun)$',$kata)){ // Jika berupa particles (“-lah”, “-kah”, “-tah” atau “-pun”)
        ...
        if (preg_match('/([km]u|nya|[kl]ah|pun)$/', $kata)) { // Cek Inflection Suffixes
            ...
            if (preg_match('/([ktl]ah|pun)$/', $kata)) { // Jika berupa particles (“-lah”, “-kah”, “-tah” atau “-pun”)
    ...
    // mark
    public function Cek_Rule_Precedence($kata) {
    ...
    public function Del_Derivation_Suffixes($kata) {
        ...
        if (preg_match('/(an|i)$/', $kata)) { // cek -kan 				
            ...
            if ($this->cek_katadasar($__kata__)) { // Cek Kamus
                return $__kata__;
            }
        }
        if (preg_match('/(kan)$/', $kata)) { // Cek Suffixes
            $__kata = preg_replace('/(kan)$/', '', $kata);
            if ($this->cek_katadasar($__kata)) { // Cek Kamus			
                return $__kata;
            }
        }
        return $kataAsal;
    ...
    public function Del_Derivation_Prefix($kata) {
        ...
        if (preg_match('/(nya)$/', $kata)) { // Cek Suffixes
            $__kata = preg_replace('/(nya)$/', '', $kata);
            if ($this->cek_katadasar($__kata)) { // Cek Kamus			
                return $__kata;
            }
        }
        ...
        if (preg_match('/^(di|[ks]e)\S{1,}/', $kata)) { // Jika di-,ke-,se-
            ...
            if ($this->cek_katadasar($__kata)) {
                return $__kata; // Jika ada balik
            }
            $__kata__ = $this->Del_Derivation_Suffixes($__kata);
            if ($this->cek_katadasar($__kata__)) {
                return $__kata__;
            }
        }
        ...
        if (preg_match('/^([tmbp]e)\S{1,}/', $kata)) { //Jika awalannya adalah “te-”, “me-”, “be-”, atau “pe-”
            ...
            if (preg_match('/^(me)\S{1,}/', $kata)) { // Jika awalan “me-”,
                ...
                if (preg_match('/^(men)[cdfjszt]\S{1,}/', $kata)) { // aturan 14.
        ...
        if (preg_match('/([km]u|nya|[kl]ah|pun)$/', $kata)) { // Cek Inflection Suffixes
            ...
            $__kata = $this->Del_Derivation_Prefix($__kata);
            if ($this->cek_katadasar($__kata)) {
                return $__kata; // Jika ada balik
            }
            if (preg_match('/([ktl]ah|pun)$/', $kata)) { // Jika berupa particles (“-lah”, “-kah”, “-tah” atau “-pun”)
                if (preg_match('/([km]u|nya)$/', $__kata)) { // Hapus Possesive Pronouns (“-ku”, “-mu”, atau “-nya”)
                    $__kata__ = preg_replace('/([km]u|nya)$/', '', $__kata);
                    $__kata__ = $this->Del_Derivation_Suffixes($__kata__);
                    $__kata__ = $this->Del_Derivation_Prefix($__kata__);
                    if ($this->cek_katadasar($__kata)) {
                        return $__kata; // Jika ada balik
                    }
                    return $__kata__;
